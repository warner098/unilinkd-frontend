...
export default function AuthModal({ isOpen, onClose, initialTab = 'login', onLoginSuccess, showToast }) {
  ...
  // AL SELECCIONAR CUENTA DE GOOGLE DESDE EL SELECTOR OFICIAL
  const handleSelectGoogleAccount = (googleAccount) => {
    ...
    if (showToast) {
      showToast(`¡Bienvenido, ${googleUserFirstName(googleAccount.nombre)}! Has ingresado con tu cuenta de Google.`, 'success', '✨');
    }
  ...
  const googleUserFirstName = (fullName) => {
  ...
  return (
    <>
      ...
      {/* MODAL SELECTOR DE CUENTAS DE GOOGLE */}
      <GoogleAccountModal
