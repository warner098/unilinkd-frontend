  ...
  const soluciones = [
    {
      ...
      problema: 'Buscar compañeros en grupos de WhatsApp suele terminar en equipos desintegrados.',
      ...
      <div className="max-w-7xl mx-auto text-center space-y-14 relative z-10">
        ...
        <div className="max-w-3xl mx-auto space-y-4">
          ...
          <h2 className="text-3xl sm:text-5xl font-black text-slate-900 tracking-tight font-heading">
            Diseñado para transformar la experiencia universitaria
          </h2>
          <p className="text-slate-600 text-base sm:text-lg font-normal max-w-2xl mx-auto">
            Centralizamos la colaboración, el talento y las oportunidades académicas en una plataforma moderna y segura.
          </p>
        </div>

        {/* GRID DE CARDS ELEVADAS */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 text-left">
          {soluciones.map((item, index) => (
            <div 
              key={index}
              className="glass-card rounded-3xl p-7 shadow-sm hover:shadow-xl hover:shadow-indigo-500/10 transition-all duration-300 flex flex-col justify-between group hover:-translate-y-1.5 border border-slate-200/80 hover:border-indigo-300"
            >
              <div className="space-y-4">
                <div className="w-14 h-14 rounded-2xl bg-gradient-to-tr from-blue-600 to-indigo-600 text-white flex items-center justify-center shadow-lg shadow-indigo-500/20 group-hover:scale-110 transition-transform duration-300">
                  {item.icono}
                </div>
                ...
                <p className="text-xs text-rose-700 font-bold bg-rose-50/80 px-3 py-2 rounded-xl border border-rose-200/60">
                  <span className="font-extrabold">Reto:</span> {item.problema}
                </p>

                <p className="text-xs text-slate-600 leading-relaxed font-normal">
                  <span className="font-extrabold text-emerald-600">Solución:</span> {item.solucion}
                </p>
              </div>
            </div>
          ))}
